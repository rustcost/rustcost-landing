...
helm upgrade --install rustcost rustcost/rustcost -n rustcost --create-namespace`;
  ...
  const [coreVersions, setCoreVersions] = useState<Ver[]>([]);
  const [feVersions, setFeVersions] = useState<Ver[]>([]);
    ...
    const loadTags = async () => {
      try {
        ...
        if (isMounted) {
          ...
          const coreTags = tags.map((tag) => ({
            version: tag.name,
            date: tag.last_updated.slice(0, 10),
            notes: `${coreRepo}/releases/tag/${tag.name}`,
          }));

          const feTags = tags.map((tag) => ({
            version: tag.name,
            date: tag.last_updated.slice(0, 10),
            notes: `${feRepo}/releases/tag/${tag.name}`,
          }));

          setCoreVersions(coreTags);
          setFeVersions(feTags);
        }
      } catch (error) {
        if (isMounted) {
            ...
            <div>
              ...
              <ul className="mt-2 list-disc pl-5 text-gray-700 dark:text-gray-300">
                <li>
                  ...
                  <a
                    className="text-blue-600 underline dark:text-amber-400"
                    href={`https://hub.docker.com/r/${coreImage}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {coreImage}
                ...
                <li>
                  ...
                  <a
                    className="text-blue-600 underline dark:text-amber-400"
                    href={`https://hub.docker.com/r/${feImage}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {feImage}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400">
                    ...
                    <a
                      className="text-blue-600 underline dark:text-amber-400"
                      href={`https://hub.docker.com/r/${section.image}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {section.image}
